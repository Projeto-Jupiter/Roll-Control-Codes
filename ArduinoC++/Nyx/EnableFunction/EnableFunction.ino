...
void setup() {
  ...
  EnableFunction enable(minAltitude, apogeeAltitude);
}

void loop() {

  mpu1.getMotion6(&ax1_Raw, &ay1_Raw, &az1_Raw, &gx1_Raw, &gy1_Raw, &gz1_Raw);
  AcX = ax1_Raw / 2048; //ver quais valores sao interessantes converter
  AcY = ay1_Raw / 2048;
  AcZ = az1_Raw / 2048;
  GyX = gx1_Raw / 16.4;
  GyY = gy1_Raw / 16.4;
  ...
  if(enable->getControlOn() == true && enable->getApogeeOn() == false) {
    //PID
  }
}
